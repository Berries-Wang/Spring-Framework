...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	 ...
	 * through introspection of the bean class.(常量，表示通过bean类的内省来确定适当的自动装配策略。)
	 *
	 * @see #setAutowireMode
	 ...
	 * 	</bean>
	 * </pre>
	 * 只有在配置文件中如此配置，propertyValues中才会有值
	 */
	@Nullable
	...
	/**
	 * 表明该Bean是否是代码合成的，合成的意思： 这不是一个应用开发人员自己定义的BeanDefinition
	 * ,而是程序自己组装而成的一个BeanDefinition，例如<aop:config>
	 */
	...
	public int getResolvedAutowireMode() {
		if (this.autowireMode == AUTOWIRE_AUTODETECT) {
			...
			/**
			 * Work out : 解决，计算出，实现 
			 * 计算出是使用setter方法来注入还是通过构造方法注入
			 * 如果拥有无参的构造函数，那么认为是通过setter方法来注入;反之，则尝试使用构造函数注入
			 *
			 */
			Constructor<?>[] constructors = getBeanClass().getConstructors();
			for (Constructor<?> constructor : constructors) {
				// 如果存在无参构造函数，那么就根据类型注入
				if (constructor.getParameterCount() == 0) {
					return AUTOWIRE_BY_TYPE;
				}
			}
			// 如果存在有参构造函数,则通过构造函数注入
