...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
	 ...
	 * BeanName -> Bean , 其中存储的是实例化之后，属性未赋值的单例对象。执行工厂方法生产出来的Bean，Bean被放进去之后，那么当Bean在创建的过程中，就可以通过getBean方法获取到
	 */
	...
	/**
	 * Return the (raw(生的；未加工的；)) singleton object registered under the given name,
	 * creating and registering a new one if none registered yet.
	 * <p>
	 * 为给定名称Bean返回一个单例对象(尚未创建完成和注册完成的)
	 *
	 * @param beanName         the name of the bean
	...
	public Object getSingleton(String beanName, ObjectFactory<?> singletonFactory) {
		Assert.notNull(beanName, "Bean name must not be null");
		synchronized (this.singletonObjects) {
			// 从第一级缓存中判断单例的Bean是否创建
			Object singletonObject = this.singletonObjects.get(beanName);
			if (singletonObject == null) {
				// 如果标志位为True,则表示在销毁单例，与此场景不符合，因此异常
				...
				/**
				 * 是否是新建的缓存，如果是，则刷新缓存(进入方法org.springframework.beans.factory.support.DefaultSingletonBeanRegistry.addSingleton)
				 * 构建一级缓存，清空第二三级缓存
				 */
				if (newSingleton) {
					addSingleton(beanName, singletonObject);
	 ...
	 * <p>
	 * this.singletonsCurrentlyInCreation.add(beanName): 标识单例正在创建
	 *
	 ...
	 * <p>
	 * 为给定的Bean注册所依赖的Bean，为的是在给定的Bean销毁前销毁掉所依赖的Bean
	 *
	 * @param beanName          the name of the bean
