...
class ConstructorResolver {
	 ...
	 * <p>
	 * “自动连接构造函数”(根据类型使用构造函数参数)行为。如果指定了显式构造函数参数值，也将应用此方法，
	 * 将所有剩余参数与来自bean工厂的bean进行匹配。
	 * 这对应于构造函数注入:在这种模式下，Spring bean工厂能够承载需要基于构造函数的依赖项解析的组件。
	 *
	...
	public BeanWrapper autowireConstructor(final String beanName, final RootBeanDefinition mbd,
		...
		// 目前的Bean从来没有被创建过，即不存在缓存
		if (constructorToUse == null) {
			// Need to resolve the constructor: 是否需要解析构造函数，注意，当构造函数不为空的时候就需要，这里没有通过注解判断
			...
			if (explicitArgs != null) {
				...
				// 根据构造函数参数，从IOC中解析参数，赋值到resolvedValues中.(一般这里返回的cargs是没有值的，即cargs.getArgumentCount()为0)
			...
			for (Constructor<?> candidate : candidates) {
				...
				if (resolvedValues != null) {
					try {
						// 处理构造函数注解: java.beans.ConstructorProperties，从注解中获取参数的Bean的名字
						String[] paramNames = ConstructorPropertiesChecker.evaluate(candidate, paramTypes.length);
						// 如果开发者没有指定
						if (paramNames == null) {
							ParameterNameDiscoverer pnd = this.beanFactory.getParameterNameDiscoverer();
							if (pnd != null) {
								// Debug发现，pnd类型: class org.springframework.core.DefaultParameterNameDiscoverer,这就是获取构造参数的名字
